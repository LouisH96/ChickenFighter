...
public class ChickenStats : MonoBehaviour
{
    ...
    private float BreedResult(float bottomValue, float maxChange, float stat1, float stat2)
    {
        float minValue;
        float maxValue;
        if (stat1 < stat2)
        ...
        float newValue = Random.Range(minValue - maxChange, maxValue + maxChange);
    ...
    {
        child._health = BreedResult(15.0f, 20.0f, this.Health, partner.Health);
        child._healthRegen = BreedResult(0.1f, 0.3f, this.HealthRegen, partner.HealthRegen);
        child._damage = BreedResult(0.5f, 1.0f, this.Damage, partner.Damage);
        child._maxSpeed = BreedResult(0.5f, 1.0f, this.MaxSpeed, partner.MaxSpeed);
        child._acceleration = BreedResult(0.5f, 1.0f, this.Acceleration, partner.Acceleration);
    }
}
